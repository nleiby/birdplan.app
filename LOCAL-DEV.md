...
That's enough to verify `up/dev-setup-fixes`, `up/travel-totals`, `up/saved-hotspot-affordances`, and `up/hotspot-modal-outside-click`. `up/targets-csv-export` and all of Wave 3 need it.
...
- At least one saved hotspot **left off** the itinerary — the dimmed-icon behavior in `up/saved-hotspot-affordances` needs the contrast
...
### `up/dev-setup-fixes`
- [ ] With `RESEND_API_KEY` unset, the backend boots and requesting a code still logs the email to stdout
- [ ] With `VITE_OPENBIRDING_API_URL` unset, `/trips` lists trips and a trip page loads (this is the regression the fix addresses)
- [ ] With both set, nothing changes

### `up/travel-totals` — day travel totals + full-route link
...
### `up/hotspot-modal-outside-click`
